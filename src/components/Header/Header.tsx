    ...
    <>
      {
        type !== 'null' && (
          <HeaderContainer type={type} isMobile={isMobile} hasSubtitle={!!subtitle}>
            {(doGoBack && location.pathname !== '/elderly_care/welcome') && <ArrowLefContainer onClick={doGoBack}><ArrowLeft /></ArrowLefContainer>}
            <TitleContainer>
              ...
              {type === 'tertiary' && <Link to="http://www.virufy.org" target="_blank"><LogoImg srcSet={`${Logo}, ${Logo2x} 2x, ${Logo3x} 3x`} src={LogoImg} size={location.pathname !== '/elderly_care/welcome' ? 'regular' : 'big'} /></Link>}
            </TitleContainer>
          </HeaderContainer>
        )
      }
